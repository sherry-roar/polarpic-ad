...
#elif defined(PUSH_SVE_INCRSORT_ORDER3) || \
        ...
        if (WarpX::current_deposition_algo == CurrentDepositionAlgo::Esirkepov) {
            ...
            if (push_type == PushType::Explicit) {
                if        (WarpX::nox == 1){
...
#ifdef MY_TEST
                    printf("Run MY_TEST\n");
                    // 测试包括： wqx, wqy, wqz, sx_m, sy_m, sz_m, j_m, k_m, l_m, jx_arr, jy_arr, jz_arr
                    std::vector<amrex::Real> wqx_test(np_to_deposit, 0.0_rt);
                    std::vector<amrex::Real> wqy_test(np_to_deposit, 0.0_rt);
                    std::vector<amrex::Real> wqz_test(np_to_deposit, 0.0_rt);
                    std::vector<amrex::Real> sx_m_test(2 * np_to_deposit, 0);
                    std::vector<amrex::Real> sy_m_test(2 * np_to_deposit, 0);
                    std::vector<amrex::Real> sz_m_test(2 * np_to_deposit, 0);
                    std::vector<int> newbin_test(np_to_deposit);
                    // std::vector<int> j_m(np_to_deposit);
                    // std::vector<int> k_m(np_to_deposit);
                    // std::vector<int> l_m(np_to_deposit);

                    amrex::FArrayBox jx_fab_test(jx_fab, amrex::make_deep_copy, 0, jx_fab.nComp());
                    amrex::FArrayBox jy_fab_test(jy_fab, amrex::make_deep_copy, 0, jy_fab.nComp());
                    amrex::FArrayBox jz_fab_test(jz_fab, amrex::make_deep_copy, 0, jz_fab.nComp());

                    // std::vector<long> xoffsets_test(8 * np_to_deposit);
                    // std::vector<long> yoffsets_test(8 * np_to_deposit);
                    // std::vector<long> zoffsets_test(8 * np_to_deposit);
                
                    // std::vector<double> jx_vals_test(8 * np_to_deposit, 0.0);
                    // std::vector<double> jy_vals_test(8 * np_to_deposit, 0.0);
                    // std::vector<double> jz_vals_test(8 * np_to_deposit, 0.0);  

                    // doDepositionShapeN_test<1>(
                    //     GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                    //     uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                    //     jx_fab_test, jy_fab_test, jz_fab_test, np_to_deposit, relative_time, dinv,
                    //     xyzmin, lo, q, 
                    //     wqx_test,wqy_test,wqz_test,
                    //     sx_m_test,sy_m_test,sz_m_test,
                    //     newbin_test,
                    //     j_m, k_m, l_m,
                    //     xoffsets_test, yoffsets_test, zoffsets_test,
                    //     jx_vals_test, jy_vals_test, jz_vals_test,
                    //     WarpX::n_rz_azimuthal_modes
                    // );

                    const Dim3 len_test = length(tilebox);
                    const long numcell = jx_fab_test.box().numPts();
                    std::vector<amrex::Real> xrhocells_test(8 * numcell, 0.0);
                    std::vector<amrex::Real> yrhocells_test(8 * numcell, 0.0);
                    std::vector<amrex::Real> zrhocells_test(8 * numcell, 0.0);
                    doDepositionShapeN_rhocell_test<1>(
                        GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                        uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                        jx_fab_test, jy_fab_test, jz_fab_test, np_to_deposit, relative_time, dinv,
                        xyzmin, len_test, q, 
                        ...
                        sx_m_test,sy_m_test,sz_m_test,
                        newbin_test,
                        xrhocells_test, yrhocells_test, zrhocells_test,
                        WarpX::n_rz_azimuthal_modes);
#endif  // MY_TEST
...
#ifdef PURE_SVE
                    // printf("Run PURE_SVE\n");
                    doDepositionShapeN_sve<1>(
                        GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                        uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                        jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                        xyzmin, lo, q, WarpX::n_rz_azimuthal_modes);
                    // doDepositionShapeN_sve_test<1>(
                    //     GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                    //     uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                    //     jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                    //     xyzmin, lo, q, 
                        ...
                        for (int iz = 0; iz <= 1; iz++){
                            for (int iy = 0; iy <= 1; iy++){
                                for (int ix = 0; ix <= 1; ix++){
                                    ...
                                    int y = lo.y + k_m[ip] + iy;
                                    int z = lo.z + l_m[ip] + iz;
                                    double jx_err = fabs(jx_arr_tmp(x, y, z) - jx_arr_test(x, y, z)) / jx_arr_test(x, y, z);
                                    double jy_err = fabs(jy_arr_tmp(x, y, z) - jy_arr_test(x, y, z)) / jy_arr_test(x, y, z);
                                    double jz_err = fabs(jz_arr_tmp(x, y, z) - jz_arr_test(x, y, z)) / jz_arr_test(x, y, z);
                                    ...
                                    {
                                        printf("ip = %ld\n", ip);
                                        printf("jx_arr: %lf, jx_arr_test: %lf, jx_err: %lf\n", jx_arr_tmp(x, y, z), jx_arr_test(x, y, z), jx_err);
                                        amrex::Abort("jx_arr is wrong\n");
                                    }
...
#ifdef ORG_SVE
                    // printf("RUN ORG_SVE\n");
                    doDepositionShapeN_org_sve<1>(
                        GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                        uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                        jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                        xyzmin, lo, q, WarpX::n_rz_azimuthal_modes);
#ifdef MY_TEST
                    // doDepositionShapeN_org_sve_test<1>(
                    //     GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                    //     uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                    //     jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                    //     xyzmin, lo, q, 
                    ...
                    for (int iz = 0; iz < len.z; ++iz)
                    {
                        for (int iy = 0; iy < len.y; ++iy)
                        {
                            for(int ix = 0; ix < len.x; ix++){
                                ...
                                {
                                    double jx_err = fabs(jx_arr.p[arr_offset + arr_step[idx]] - jx_arr_test.p[arr_offset + arr_step[idx]]) / jx_arr_test.p[arr_offset + arr_step[idx]];
                                    double jy_err = fabs(jy_arr.p[arr_offset + arr_step[idx]] - jy_arr_test.p[arr_offset + arr_step[idx]]) / jy_arr_test.p[arr_offset + arr_step[idx]];
                                    double jz_err = fabs(jz_arr.p[arr_offset + arr_step[idx]] - jz_arr_test.p[arr_offset + arr_step[idx]]) / jz_arr_test.p[arr_offset + arr_step[idx]];

                                    if (jx_err > 1e-9)
                                    {
                                        printf("jx_arr: %lf, jx_arr_test: %lf, jx_err: %lf\n", jx_arr.p[arr_offset + arr_step[idx]], jx_arr_test.p[arr_offset + arr_step[idx]], jx_err);
                                        amrex::Abort("jx_arr is wrong\n");
                                    }
                                    if (jy_err > 1e-9)
                                    {
                                        printf("jy_arr: %lf, jy_arr_test: %lf, jy_err: %lf\n", jy_arr.p[arr_offset + arr_step[idx]], jy_arr_test.p[arr_offset + arr_step[idx]], jy_err);
                                        amrex::Abort("jy_arr is wrong\n");
                                    }
                                    if (jz_err > 1e-9)
                                    {
                                        printf("jz_arr: %lf, jz_arr_test: %lf, jz_err: %lf\n", jx_arr.p[arr_offset + arr_step[idx]], jz_arr_test.p[arr_offset + arr_step[idx]], jz_err);
                                        amrex::Abort("jz_arr is wrong\n");
                                    }
                                }
                            }
...
#ifdef SVE_RHOCELL
                    // printf("RUN SVE_RHOCELL\n");
                    const Dim3 len = length(tilebox);
                    doDepositionShapeN_sve_rhocell<1>(
                        GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                        uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                        jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                        xyzmin, len, q, WarpX::n_rz_azimuthal_modes);
...
#ifdef MY_TEST
                    // doDepositionShapeN_sve_rhocell_test<1>(
                    //     GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                    //     uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                    //     jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                    //     xyzmin, len_test, q, 
...
#ifdef MY_TEST
                    // doDepositionShapeN_sve_sme_test<1>(
                    //     GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                    //     uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                    //     jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                    //     xyzmin, lo, q, 
                        ...
                        for (int iy = 0; iy < len.y; ++iy)
                        {
                            for(int ix = 0; ix < len.x; ix++){
                                ...
                                for (int idx = 0; idx < 8; ++idx)
                                {
                                    ...
                                    if (jz_err > 1e-8)
                                    {
                                        printf("jz_arr: %lf, jz_arr_test: %lf, jz_err: %lf\n", jz_arr.p[arr_offset + arr_step[idx]], jz_arr_test.p[arr_offset + arr_step[idx]], jz_err);
                                        amrex::Abort("jz_arr is wrong\n");
                                    }
                                }
                            }
...
#ifdef SVE_RHOCELL_SME
                    // printf("RUN SVE_RHOCELL_SME\n");
                    const Dim3 len = length(tilebox);
                    doDepositionShapeN_sve_rhocell_sme<1>(
                        GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                        uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                        jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                        xyzmin, len, q, WarpX::n_rz_azimuthal_modes);
#ifdef MY_TEST
                    // doDepositionShapeN_sve_rhocell_sme_test<1>(
                    //         GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                    //         uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                    //         jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                    //         xyzmin, len_test, q, 
                    ...
                    printf("Run MY_TEST_ORDER3\n");
                    // 测试包括： wqx, wqy, wqz, sx_m, sy_m, sz_m, j_m, k_m, l_m, jx_arr, jy_arr, jz_arr
                    std::vector<amrex::Real> wqx_test(np_to_deposit, 0.0_rt);
                    std::vector<amrex::Real> wqy_test(np_to_deposit, 0.0_rt);
                    std::vector<amrex::Real> wqz_test(np_to_deposit, 0.0_rt);
                    std::vector<amrex::Real> sx_m_test(4 * np_to_deposit, 0);
                    std::vector<amrex::Real> sy_m_test(4 * np_to_deposit, 0);
                    std::vector<amrex::Real> sz_m_test(4 * np_to_deposit, 0);
                    std::vector<int> newbin_test(np_to_deposit);
                    std::vector<int> j_m(np_to_deposit);
                    std::vector<int> k_m(np_to_deposit);
                    std::vector<int> l_m(np_to_deposit);
                    ...
                    std::vector<long> xoffsets_test(64 * np_to_deposit);
                    std::vector<long> yoffsets_test(64 * np_to_deposit);
                    std::vector<long> zoffsets_test(64 * np_to_deposit);
                
                    std::vector<double> jx_vals_test(64 * np_to_deposit, 0.0);
                    std::vector<double> jy_vals_test(64 * np_to_deposit, 0.0);
                    std::vector<double> jz_vals_test(64 * np_to_deposit, 0.0);  

                    doDepositionShapeN_order3_test<3>(
                        GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                        uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                        jx_fab_test, jy_fab_test, jz_fab_test, np_to_deposit, relative_time, dinv,
                        xyzmin, lo, q, 
                    ...
                    // printf("RUN ORG_SVE_ORDER3\n");
                    doDepositionShapeN_org_sve_order3<3>(
                        GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                        uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                        jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                        xyzmin, lo, q, WarpX::n_rz_azimuthal_modes);
                    ...
                    // printf("RUN PURE_SVE_ORDER3\n");
                    doDepositionShapeN_sve_order3<3>(
                        GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                        uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                        jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                        xyzmin, lo, q, WarpX::n_rz_azimuthal_modes);
                    ...
                    // printf("RUN PURE_RHOCELL_ORDER3\n");
                    const Dim3 len = length(tilebox);
                    doDepositionShapeN_rhocell_order3<3>(
                        GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                        uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                        jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                        xyzmin, len, q, WarpX::n_rz_azimuthal_modes);
                    ...
                    Box real_box;
                    if (lev == depos_lev) {
                        real_box = pti.tilebox();
                    } else {
                        const IntVect& ref_ratio = WarpX::RefRatio(depos_lev);
                        real_box = amrex::coarsen(pti.tilebox(),ref_ratio);
                    }

                    const Dim3 len = length(tilebox);
                    auto& ptile = ParticlesAt(lev, pti);
                    // doDepositionShapeN_sve_rhocell_incrsort_order3<3>(
                    doDepositionShapeN_sve_rhocell_incrsort_order3_eurosys<3>(
                        GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                        uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                        jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                        xyzmin, len, real_box, tilebox, q, ptile, WarpX::n_rz_azimuthal_modes
                    );
                    ...
                    const Dim3 len = length(tilebox);
                    auto& ptile = ParticlesAt(lev, pti);
                    // doDepositionShapeN_sve_rhocell_sme_incrsort_order3<3>(
                    doDepositionShapeN_sve_rhocell_sme_incrsort_order3_eurosys<3>(
                        GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                        uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                        jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                        xyzmin, len, real_box, tilebox, q, ptile, WarpX::n_rz_azimuthal_modes
                    ...
                    const Dim3 len = length(tilebox);
                    auto& ptile = ParticlesAt(lev, pti);
                    // doDepositionShapeN_sve_rhocell_binsort_physort_order3<3>(
                    doDepositionShapeN_sve_rhocell_binsort_physort_order3_eurosys<3>(
                        GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                        uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                        jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                    ...
                    doDepositionShapeN_sve_rhocell_sme_binsort_physort_order3_eurosys<3>(
                        GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                        uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                        jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                        xyzmin, len, q, ptile, WarpX::n_rz_azimuthal_modes
                    ...
                    const Dim3 len = length(tilebox);
                    auto& ptile = ParticlesAt(lev, pti);
                    doDepositionShapeN_sve_rhocell_physort_order3<3>(
                        GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                        uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                        jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                        xyzmin, len, q, ptile, WarpX::n_rz_azimuthal_modes
                    ...
                    const Dim3 len = length(tilebox);
                    auto& ptile = ParticlesAt(lev, pti);
                    doDepositionShapeN_sve_rhocell_sme_physort_order3<3>(
                        GetPosition, wp.dataPtr() + offset, uxp.dataPtr() + offset,
                        uyp.dataPtr() + offset, uzp.dataPtr() + offset, ion_lev,
                        jx_fab, jy_fab, jz_fab, np_to_deposit, relative_time, dinv,
                        xyzmin, len, q, ptile, WarpX::n_rz_azimuthal_modes
