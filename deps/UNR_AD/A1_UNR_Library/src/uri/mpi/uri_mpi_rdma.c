...
uru_status_t uri_mpi_rdma_plan_create(
    ...
    *plan_pp = plan_p;
    ...
    for (int i = 0; i < num_wrt; i++) {
        ...
        if (loc_offset_arr[i] + wrt_size_arr[i] > send_mem_h_size) {
            URU_PRINT_INT64("loc_offset_arr[i]", loc_offset_arr[i]);
            URU_PRINT_INT64("wrt_size_arr[i]", wrt_size_arr[i]);
            URU_PRINT_INT64("send_mem_h_size", send_mem_h_size);
        }
        URU_ASSERT(loc_offset_arr[i] + wrt_size_arr[i] <= send_mem_h_size);
        URU_ASSERT(rmt_offset_arr[i] + wrt_size_arr[i] <= recv_mem_h_size);
        send_op->addr = send_mem_h_addr + loc_offset_arr[i];
        recv_op->addr = recv_mem_h_addr + rmt_offset_arr[i];
        send_op->size = wrt_size_arr[i];
        recv_op->size = wrt_size_arr[i];
