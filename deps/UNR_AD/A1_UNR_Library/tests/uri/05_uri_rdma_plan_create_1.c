 ...
 * @brief This file mainly tests \ref uri_rdma_plan_create with `once_now=1`
 * @attention `mem_size=16384` is different from \ref 04_uri_rdma_plan_create_0.c
 *            because non-persist MPI operation is extremely slow in this communication pattern.
 * @version 1.0
 * @date 2024-02-13
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < uri_num_channels; i++) {
        ...
        for (int mem_size = 2; mem_size <= 16384; mem_size *= 2) {
            ...
            uint64_t* wrt_size_arr = (uint64_t*)uru_malloc(num_wrt * sizeof(uint64_t));
            ...
            uri_rdma_plan_create(
                uri_channel_arr[i], 1, num_wrt,
                loc_mem_h_arr, loc_offset_arr, loc_flag_p_arr, loc_flag_add_arr, wrt_size_arr,
