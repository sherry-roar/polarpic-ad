...
int main(int argc, char** argv) {
    ...
    for (int mi = 0, msg_size = 1; msg_size <= max_msg_size; msg_size *= 2, mi++) {
        ...
        unr_blk_part_send_plan(send_blk_h, UNR_NO_SIGNAL, 0, msg_size, rmt_blk_h, rmt_sig[1], 0, plan_h+0);
        unr_blk_part_send_plan(send_blk_h, UNR_NO_SIGNAL, 0, msg_size, rmt_blk_h, rmt_sig[0], 0, plan_h+1);
