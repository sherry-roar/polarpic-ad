module mod_mesh
    ...
    !@cuf attributes(managed) :: dzc, dzf, dzc_inv, dzf_inv
    !@cuf attributes(managed) :: dzf_global, dzflzi, visc_dzf_inv
    ...
    subroutine initMesh()
        ...
        !@cuf istat = cudaMemAdvise( dzc_inv, size(dzc_inv), cudaMemAdviseSetReadMostly, 0 )
        !@cuf istat = cudaMemAdvise( dzf_inv, size(dzf_inv), cudaMemAdviseSetReadMostly, 0 )
        !@cuf istat = cudaMemAdvise( dzf_global, size(dzf_global), cudaMemAdviseSetReadMostly, 0 )
        !@cuf istat = cudaMemAdvise( dzflzi, size(dzflzi), cudaMemAdviseSetReadMostly, 0 )
        !@cuf istat = cudaMemAdvise( visc_dzf_inv, size(visc_dzf_inv), cudaMemAdviseSetReadMostly, 0 )
        ...
        !@cuf istat = cudaMemPrefetchAsync( dzc_inv, size(dzc_inv), mydev, 0 )
        !@cuf istat = cudaMemPrefetchAsync( dzf_inv, size(dzf_inv), mydev, 0 )
        !@cuf istat = cudaMemPrefetchAsync( dzf_global, size(dzf_global), mydev, 0 )
        !@cuf istat = cudaMemPrefetchAsync( dzflzi, size(dzflzi), mydev, 0 )
        !@cuf istat = cudaMemPrefetchAsync( visc_dzf_inv, size(visc_dzf_inv), mydev, 0 )
