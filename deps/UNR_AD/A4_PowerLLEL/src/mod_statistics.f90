module mod_statistics
    ...
    use mod_mesh,     only: dx_inv, dy_inv, dzc_inv, dzf_inv
    use mod_dataIO,   only: inputLine, outputLine, inputPlane, outputPlane
    use mod_spaceAvg, only: avg_plane_t, initAvgPlane, reduceToAvgPlane, recoverFromAvgPlane, freeAvgPlane, &
                            avg_line_t, initAvgLine, reduceToAvgLine, recoverFromAvgLine, freeAvgLine
    use mod_mpi,      only: coord_xpen, coord_ypen, xsz, xst, ysz, yst, myrank
#ifndef USE_C
    use mod_poissonSolver, only: var_xpen, var_ypen
    use decomp_2d,    only: transpose_x_to_y
#else
    use decomp_2d_c,  only: decomp_main, transpose_x_to_y_real
#endif
    ...
    type(stat_info_t), save, public :: stat_info

    real(fp), parameter :: r1 =  9.0_fp/8.0_fp
    real(fp), parameter :: r2 = -1.0_fp/8.0_fp
    real(fp), save :: r1interp, r2interp, r1dxi, r2dxi, r1dyi, r2dyi
    ...
    real(fp_pois),    allocatable, dimension(:), save :: wrk_xpen_1d, wrk_ypen_1d
    complex(fp_pois), allocatable, dimension(:), save :: wrk_xpen_1d_c, wrk2_xpen_1d_c
    complex(fp_pois), allocatable, dimension(:), save :: wrk_ypen_1d_c, wrk2_ypen_1d_c
    real(fp_pois),    allocatable, dimension(:,:,:), save :: var2_xpen, var2_ypen
    
#ifdef USE_C
    type(C_PTR), save :: var_xpen_cptr, var_ypen_cptr
    real(fp_pois), pointer, dimension(:,:,:), save :: var_xpen, var_ypen
    interface
#if defined(_PDD) && defined(USE_RDMA)
    subroutine borrow_poisson_solver_buffer_xy() bind(C, name='borrow_poisson_solver_buffer_xy')
        import
    end subroutine
    subroutine return_poisson_solver_buffer_xy() bind(C, name='return_poisson_solver_buffer_xy')
        import
    end subroutine
#endif
    type(C_PTR) function getPoissonSolverBufferXpen() bind(C, name='get_poisson_solver_buffer_xpen')
        import
    end function
    type(C_PTR) function getPoissonSolverBufferYpen() bind(C, name='get_poisson_solver_buffer_ypen')
        import
    end function
    end interface
#endif
    ...
    character(8), save :: fn_prefix_al_z = "avg1d-z_"
    character(9), save :: fn_prefix_ap_xz = "avg2d-xz_"
    character(9), save :: fn_prefix_ap_yz = "avg2d-yz_"

    integer, save :: nx, ny, nz
    integer, save :: ie, je, ke

    public :: allocStat, freeStat, initStat, calcStat, inputStat, outputStat

contains
    subroutine allocStat(sz_global, sz)
        implicit none
        integer, dimension(3), intent(in) :: sz_global, sz
...
#ifdef USE_C
        var_xpen_cptr = getPoissonSolverBufferXpen()
        var_ypen_cptr = getPoissonSolverBufferYpen()
        call c_f_pointer(var_xpen_cptr, var_xpen, shape=xsz)
        call c_f_pointer(var_ypen_cptr, var_ypen, shape=ysz)
        if ((.not. c_associated(var_xpen_cptr)) .and. myrank == 0) write(*, '(A)') ">>> var_xpen_cptr is null!"
        if ((.not. c_associated(var_ypen_cptr)) .and. myrank == 0) write(*, '(A)') ">>> var_ypen_cptr is null!"
        if ((.not. c_associated(var_xpen_cptr, c_loc(var_xpen))) .and. myrank == 0) write(*, '(A)') ">>> var_xpen_cptr & var_xpen point to different addresses!"
        if ((.not. c_associated(var_ypen_cptr, c_loc(var_ypen))) .and. myrank == 0) write(*, '(A)') ">>> var_ypen_cptr & var_ypen point to different addresses!"
#endif
    ...
    subroutine calcStat(stat_scheme, nt, nhalo, u, v, w, nhalo_p, p, u_crf)
        implicit none
        integer,  intent(in) :: stat_scheme, nt
        integer, dimension(6), intent(in) :: nhalo, nhalo_p
        real(fp), dimension(1-nhalo(1):,1-nhalo(3):,1-nhalo(5):), intent(in) :: u, v, w
        real(fp), dimension(1-nhalo_p(1):,1-nhalo_p(3):,1-nhalo_p(5):), intent(in) :: p
        real(fp), intent(in) :: u_crf

        stat_info%nte  = nt
        stat_info%nspl = stat_info%nspl + 1
        
        select case(stat_scheme)
        case(1)
            call calcStatScheme1(nt, nhalo, u, v, w, nhalo_p, p, u_crf)
        case(2)
            call calcStatScheme2(nt, nhalo, u, v, w, nhalo_p, p, u_crf)
        end select
        
        return
    end subroutine calcStat

    subroutine calcStatScheme1(nt, nhalo, u, v, w, nhalo_p, p, u_crf)
        implicit none
        ...
        do i = 1, ie
            ...
            wf  = w(i,j,k)
            wfm = w(i,j,k-1)
            uc = (u(i-1,j,k)+u(i,j,k))*0.5_fp + u_crf
...
#if defined(USE_C) && defined(_PDD) && defined(USE_RDMA)
        call borrow_poisson_solver_buffer_xy()
#endif

        call calcEnergySpectra_kx(nhalo, u, w, euu_kx_stat, eww_kx_stat, euw_kx_stat, u_crf)
        call calcEnergySpectra_ky(nhalo, u, w, euu_ky_stat, eww_ky_stat, euw_ky_stat, u_crf)
        ! call calcEnergySpectra_kx(nhalo, u, euu_kx_stat, u_crf)
        ! call calcEnergySpectra_ky(nhalo, u, euu_ky_stat, u_crf)
...
#if defined(USE_C) && defined(_PDD) && defined(USE_RDMA)
        call return_poisson_solver_buffer_xy()
#endif

        return
    end subroutine calcStatScheme1

    subroutine calcStatScheme2(nt, nhalo, u, v, w, nhalo_p, p, u_crf)
        implicit none
        integer,  intent(in) :: nt
        integer, dimension(6), intent(in) :: nhalo, nhalo_p
        real(fp), dimension(1-nhalo(1):,1-nhalo(3):,1-nhalo(5):), intent(in) :: u, v, w
        ...
        do i = 1, ie
            ...
            wf  = w(i,j,k)               ! zf
            wfm = w(i,j,k-1)
            uc = r1interp*(u(i-1, j, k) + u(i, j, k)) + r2interp*(u(i-2, j, k) + u(i+1, j, k)) + u_crf
            ...
            dudx  = (u(i, j  , k) - u(i-1, j, k))*r1dxi + (u(i+1, j  , k) - u(i-2, j  , k))*r2dxi
            dudy  = (u(i, j+1, k) - u(i  , j, k))*r1dyi + (u(i  , j+2, k) - u(i  , j-1, k))*r2dyi
            dudz  = (u(i, j, k+1) - u(i, j, k  ))*dzc_inv(k  )               ! zf
            dudzm = (u(i, j, k  ) - u(i, j, k-1))*dzc_inv(k-1)
            dvdx  = (v(i+1, j, k) - v(i, j, k  ))*r1dxi + (v(i+2, j  , k) - v(i-1, j  , k))*r2dxi
            dvdy  = (v(i  , j, k) - v(i, j-1, k))*r1dyi + (v(i  , j+1, k) - v(i  , j-2, k))*r2dyi
            dvdz  = (v(i, j, k+1) - v(i, j, k  ))*dzc_inv(k  )               ! zf
            dvdzm = (v(i, j, k  ) - v(i, j, k-1))*dzc_inv(k-1)
            dwdx  = (w(i+1, j, k  ) - w(i, j, k  ))*r1dxi + (w(i+2, j, k  ) - w(i-1, j, k  ))*r2dxi
            dwdxm = (w(i+1, j, k-1) - w(i, j, k-1))*r1dxi + (w(i+2, j, k-1) - w(i-1, j, k-1))*r2dxi
            dwdy  = (w(i, j+1, k  ) - w(i, j, k  ))*r1dyi + (w(i, j+2, k  ) - w(i, j-1, k  ))*r2dyi
            dwdym = (w(i, j+1, k-1) - w(i, j, k-1))*r1dyi + (w(i, j+2, k-1) - w(i, j-1, k-1))*r2dyi
            dwdz  = (w(i, j, k) - w(i, j, k-1))*dzf_inv(k)
    ...
    subroutine stripHalo(nhalo, sz, var_halo, var)
        ...
        real(fp_pois), dimension( :, :, :), intent(out) :: var
        ...
        do j = 1, sz(2)
        do i = 1, sz(1)
            var(i, j, k) = real(var_halo(i, j, k), fp_pois)
        enddo
        enddo
        enddo
        !$OMP END PARALLEL DO
    ...
    subroutine stripHaloAndAddConstant(nhalo, sz, var_halo, var, const)
        ...
        do j = 1, sz(2)
        do i = 1, sz(1)
            var(i, j, k) = real(var_halo(i, j, k) + const, fp_pois)
        enddo
        enddo
        enddo
        !$OMP END PARALLEL DO
...
#else
        call transpose_x_to_y_real(var_xpen, var_ypen, xsz, ysz, c_loc(decomp_main))
#endif
...
#else
        call transpose_x_to_y_real(var_xpen, var2_ypen, xsz, ysz, c_loc(decomp_main))
#endif
    ...
    subroutine inputStat(fn_prefix_input_stat)
        implicit none
        character(*), intent(in) :: fn_prefix_input_stat
        character(len=:), allocatable :: fn_prefix
        integer :: ng1, st1, sz1, nh1(2)
        integer :: ng2(2), st2(2), sz2(2), nh2(4)
        integer :: nt = -1

        if (myrank == 0) write(*,'(A)') "PowerLLEL.NOTE.inputStat: Reading checkpoint fields of statistics ..."
        
        ng1 = nz
        st1 = xst(3)
        sz1 = xsz(3)
        nh1 = (/0, 0/)
        fn_prefix = fn_prefix_al_z//trim(fn_prefix_input_stat)//'_'

        call inputLine(fn_prefix//'u.h5', .true., nt, ng1, st1, sz1, nh1, 'u_stat',  al_z%buffer, stat_info)
        call recoverFromAvgLine(u_stat, al_z)
        call inputLine(fn_prefix//'v.h5', .true., nt, ng1, st1, sz1, nh1, 'v_stat',  al_z%buffer, stat_info)
        call recoverFromAvgLine(v_stat, al_z)
        call inputLine(fn_prefix//'w.h5', .true., nt, ng1, st1, sz1, nh1, 'w_stat',  al_z%buffer, stat_info)
        call recoverFromAvgLine(w_stat, al_z)
        call inputLine(fn_prefix//'u2.h5', .true., nt, ng1, st1, sz1, nh1, 'u2_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(u2_stat, al_z)
        call inputLine(fn_prefix//'v2.h5', .true., nt, ng1, st1, sz1, nh1, 'v2_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(v2_stat, al_z)
        call inputLine(fn_prefix//'w2.h5', .true., nt, ng1, st1, sz1, nh1, 'w2_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(w2_stat, al_z)
        call inputLine(fn_prefix//'uv.h5', .true., nt, ng1, st1, sz1, nh1, 'uv_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(uv_stat, al_z)
        call inputLine(fn_prefix//'uw.h5', .true., nt, ng1, st1, sz1, nh1, 'uw_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(uw_stat, al_z)
        call inputLine(fn_prefix//'vw.h5', .true., nt, ng1, st1, sz1, nh1, 'vw_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(vw_stat, al_z)
        call inputLine(fn_prefix//'p.h5', .true., nt, ng1, st1, sz1, nh1, 'p_stat',  al_z%buffer, stat_info)
        call recoverFromAvgLine(p_stat, al_z)
        call inputLine(fn_prefix//'p2.h5', .true., nt, ng1, st1, sz1, nh1, 'p2_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(p2_stat, al_z)
        
        call inputLine(fn_prefix//'u3.h5', .true., nt, ng1, st1, sz1, nh1, 'u3_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(u3_stat, al_z)
        call inputLine(fn_prefix//'v3.h5', .true., nt, ng1, st1, sz1, nh1, 'v3_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(v3_stat, al_z)
        call inputLine(fn_prefix//'w3.h5', .true., nt, ng1, st1, sz1, nh1, 'w3_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(w3_stat, al_z)
        call inputLine(fn_prefix//'u4.h5', .true., nt, ng1, st1, sz1, nh1, 'u4_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(u4_stat, al_z)
        call inputLine(fn_prefix//'v4.h5', .true., nt, ng1, st1, sz1, nh1, 'v4_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(v4_stat, al_z)
        call inputLine(fn_prefix//'w4.h5', .true., nt, ng1, st1, sz1, nh1, 'w4_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(w4_stat, al_z)
        call inputLine(fn_prefix//'p3.h5', .true., nt, ng1, st1, sz1, nh1, 'p3_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(p3_stat, al_z)
        call inputLine(fn_prefix//'p4.h5', .true., nt, ng1, st1, sz1, nh1, 'p4_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(p4_stat, al_z)
        
        call inputLine(fn_prefix//'dudx.h5', .true., nt, ng1, st1, sz1, nh1, 'dudx_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dudx_stat, al_z)
        call inputLine(fn_prefix//'dudy.h5', .true., nt, ng1, st1, sz1, nh1, 'dudy_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dudy_stat, al_z)
        call inputLine(fn_prefix//'dudz.h5', .true., nt, ng1, st1, sz1, nh1, 'dudz_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dudz_stat, al_z)
        call inputLine(fn_prefix//'dvdx.h5', .true., nt, ng1, st1, sz1, nh1, 'dvdx_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dvdx_stat, al_z)
        call inputLine(fn_prefix//'dvdy.h5', .true., nt, ng1, st1, sz1, nh1, 'dvdy_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dvdy_stat, al_z)
        call inputLine(fn_prefix//'dvdz.h5', .true., nt, ng1, st1, sz1, nh1, 'dvdz_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dvdz_stat, al_z)
        call inputLine(fn_prefix//'dwdx.h5', .true., nt, ng1, st1, sz1, nh1, 'dwdx_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dwdx_stat, al_z)
        call inputLine(fn_prefix//'dwdy.h5', .true., nt, ng1, st1, sz1, nh1, 'dwdy_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dwdy_stat, al_z)
        call inputLine(fn_prefix//'dwdz.h5', .true., nt, ng1, st1, sz1, nh1, 'dwdz_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dwdz_stat, al_z)
        
        call inputLine(fn_prefix//'dudx_pow2.h5', .true., nt, ng1, st1, sz1, nh1, 'dudx_pow2_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dudx_pow2_stat, al_z)
        call inputLine(fn_prefix//'dudy_pow2.h5', .true., nt, ng1, st1, sz1, nh1, 'dudy_pow2_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dudy_pow2_stat, al_z)
        call inputLine(fn_prefix//'dudz_pow2.h5', .true., nt, ng1, st1, sz1, nh1, 'dudz_pow2_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dudz_pow2_stat, al_z)
        call inputLine(fn_prefix//'dvdx_pow2.h5', .true., nt, ng1, st1, sz1, nh1, 'dvdx_pow2_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dvdx_pow2_stat, al_z)
        call inputLine(fn_prefix//'dvdy_pow2.h5', .true., nt, ng1, st1, sz1, nh1, 'dvdy_pow2_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dvdy_pow2_stat, al_z)
        call inputLine(fn_prefix//'dvdz_pow2.h5', .true., nt, ng1, st1, sz1, nh1, 'dvdz_pow2_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dvdz_pow2_stat, al_z)
        call inputLine(fn_prefix//'dwdx_pow2.h5', .true., nt, ng1, st1, sz1, nh1, 'dwdx_pow2_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dwdx_pow2_stat, al_z)
        call inputLine(fn_prefix//'dwdy_pow2.h5', .true., nt, ng1, st1, sz1, nh1, 'dwdy_pow2_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dwdy_pow2_stat, al_z)
        call inputLine(fn_prefix//'dwdz_pow2.h5', .true., nt, ng1, st1, sz1, nh1, 'dwdz_pow2_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dwdz_pow2_stat, al_z)
        
        call inputLine(fn_prefix//'dudy_dvdx.h5', .true., nt, ng1, st1, sz1, nh1, 'dudy_dvdx_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dudy_dvdx_stat, al_z)
        call inputLine(fn_prefix//'dudz_dwdx.h5', .true., nt, ng1, st1, sz1, nh1, 'dudz_dwdx_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dudz_dwdx_stat, al_z)
        call inputLine(fn_prefix//'dvdz_dwdy.h5', .true., nt, ng1, st1, sz1, nh1, 'dvdz_dwdy_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dvdz_dwdy_stat, al_z)


        call inputLine(fn_prefix//'u5.h5', .true., nt, ng1, st1, sz1, nh1, 'u5_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(u5_stat, al_z)
        call inputLine(fn_prefix//'u6.h5', .true., nt, ng1, st1, sz1, nh1, 'u6_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(u6_stat, al_z)
        call inputLine(fn_prefix//'u7.h5', .true., nt, ng1, st1, sz1, nh1, 'u7_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(u7_stat, al_z)
        call inputLine(fn_prefix//'u8.h5', .true., nt, ng1, st1, sz1, nh1, 'u8_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(u8_stat, al_z)
        call inputLine(fn_prefix//'u9.h5', .true., nt, ng1, st1, sz1, nh1, 'u9_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(u9_stat, al_z)
        call inputLine(fn_prefix//'u10.h5', .true., nt, ng1, st1, sz1, nh1, 'u10_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(u10_stat, al_z)
        call inputLine(fn_prefix//'v5.h5', .true., nt, ng1, st1, sz1, nh1, 'v5_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(v5_stat, al_z)
        call inputLine(fn_prefix//'v6.h5', .true., nt, ng1, st1, sz1, nh1, 'v6_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(v6_stat, al_z)
        call inputLine(fn_prefix//'v7.h5', .true., nt, ng1, st1, sz1, nh1, 'v7_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(v7_stat, al_z)
        call inputLine(fn_prefix//'v8.h5', .true., nt, ng1, st1, sz1, nh1, 'v8_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(v8_stat, al_z)
        call inputLine(fn_prefix//'v9.h5', .true., nt, ng1, st1, sz1, nh1, 'v9_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(v9_stat, al_z)
        call inputLine(fn_prefix//'v10.h5', .true., nt, ng1, st1, sz1, nh1, 'v10_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(v10_stat, al_z)
        call inputLine(fn_prefix//'w5.h5', .true., nt, ng1, st1, sz1, nh1, 'w5_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(w5_stat, al_z)
        call inputLine(fn_prefix//'w6.h5', .true., nt, ng1, st1, sz1, nh1, 'w6_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(w6_stat, al_z)
        call inputLine(fn_prefix//'w7.h5', .true., nt, ng1, st1, sz1, nh1, 'w7_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(w7_stat, al_z)
        call inputLine(fn_prefix//'w8.h5', .true., nt, ng1, st1, sz1, nh1, 'w8_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(w8_stat, al_z)
        call inputLine(fn_prefix//'w9.h5', .true., nt, ng1, st1, sz1, nh1, 'w9_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(w9_stat, al_z)
        call inputLine(fn_prefix//'w10.h5', .true., nt, ng1, st1, sz1, nh1, 'w10_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(w10_stat, al_z)
        call inputLine(fn_prefix//'p5.h5', .true., nt, ng1, st1, sz1, nh1, 'p5_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(p5_stat, al_z)
        call inputLine(fn_prefix//'p6.h5', .true., nt, ng1, st1, sz1, nh1, 'p6_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(p6_stat, al_z)
        call inputLine(fn_prefix//'p7.h5', .true., nt, ng1, st1, sz1, nh1, 'p7_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(p7_stat, al_z)
        call inputLine(fn_prefix//'p8.h5', .true., nt, ng1, st1, sz1, nh1, 'p8_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(p8_stat, al_z)
        call inputLine(fn_prefix//'p9.h5', .true., nt, ng1, st1, sz1, nh1, 'p9_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(p9_stat, al_z)
        call inputLine(fn_prefix//'p10.h5', .true., nt, ng1, st1, sz1, nh1, 'p10_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(p10_stat, al_z)

        call inputLine(fn_prefix//'dudz_pow3.h5', .true., nt, ng1, st1, sz1, nh1, 'dudz_pow3_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dudz_pow3_stat, al_z)
        call inputLine(fn_prefix//'dudz_pow4.h5', .true., nt, ng1, st1, sz1, nh1, 'dudz_pow4_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dudz_pow4_stat, al_z)
        call inputLine(fn_prefix//'dudz_pow5.h5', .true., nt, ng1, st1, sz1, nh1, 'dudz_pow5_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dudz_pow5_stat, al_z)
        call inputLine(fn_prefix//'dudz_pow6.h5', .true., nt, ng1, st1, sz1, nh1, 'dudz_pow6_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dudz_pow6_stat, al_z)
        call inputLine(fn_prefix//'dudz_pow7.h5', .true., nt, ng1, st1, sz1, nh1, 'dudz_pow7_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dudz_pow7_stat, al_z)
        call inputLine(fn_prefix//'dudz_pow8.h5', .true., nt, ng1, st1, sz1, nh1, 'dudz_pow8_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dudz_pow8_stat, al_z)
        call inputLine(fn_prefix//'dudz_pow9.h5', .true., nt, ng1, st1, sz1, nh1, 'dudz_pow9_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dudz_pow9_stat, al_z)
        call inputLine(fn_prefix//'dudz_pow10.h5', .true., nt, ng1, st1, sz1, nh1, 'dudz_pow10_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dudz_pow10_stat, al_z)
        call inputLine(fn_prefix//'dvdz_pow3.h5', .true., nt, ng1, st1, sz1, nh1, 'dvdz_pow3_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dvdz_pow3_stat, al_z)
        call inputLine(fn_prefix//'dvdz_pow4.h5', .true., nt, ng1, st1, sz1, nh1, 'dvdz_pow4_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dvdz_pow4_stat, al_z)
        call inputLine(fn_prefix//'dvdz_pow5.h5', .true., nt, ng1, st1, sz1, nh1, 'dvdz_pow5_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dvdz_pow5_stat, al_z)
        call inputLine(fn_prefix//'dvdz_pow6.h5', .true., nt, ng1, st1, sz1, nh1, 'dvdz_pow6_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dvdz_pow6_stat, al_z)
        call inputLine(fn_prefix//'dvdz_pow7.h5', .true., nt, ng1, st1, sz1, nh1, 'dvdz_pow7_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dvdz_pow7_stat, al_z)
        call inputLine(fn_prefix//'dvdz_pow8.h5', .true., nt, ng1, st1, sz1, nh1, 'dvdz_pow8_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dvdz_pow8_stat, al_z)
        call inputLine(fn_prefix//'dvdz_pow9.h5', .true., nt, ng1, st1, sz1, nh1, 'dvdz_pow9_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dvdz_pow9_stat, al_z)
        call inputLine(fn_prefix//'dvdz_pow10.h5', .true., nt, ng1, st1, sz1, nh1, 'dvdz_pow10_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(dvdz_pow10_stat, al_z)

        
        call inputLine(fn_prefix//'omega_x.h5', .true., nt, ng1, st1, sz1, nh1, 'omega_x_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(omega_x_stat, al_z)
        call inputLine(fn_prefix//'omega_y.h5', .true., nt, ng1, st1, sz1, nh1, 'omega_y_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(omega_y_stat, al_z)
        call inputLine(fn_prefix//'omega_z.h5', .true., nt, ng1, st1, sz1, nh1, 'omega_z_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(omega_z_stat, al_z)
        call inputLine(fn_prefix//'omega_x2.h5', .true., nt, ng1, st1, sz1, nh1, 'omega_x2_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(omega_x2_stat, al_z)
        call inputLine(fn_prefix//'omega_y2.h5', .true., nt, ng1, st1, sz1, nh1, 'omega_y2_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(omega_y2_stat, al_z)
        call inputLine(fn_prefix//'omega_z2.h5', .true., nt, ng1, st1, sz1, nh1, 'omega_z2_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(omega_z2_stat, al_z)
        call inputLine(fn_prefix//'omega_xy.h5', .true., nt, ng1, st1, sz1, nh1, 'omega_xy_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(omega_xy_stat, al_z)
        call inputLine(fn_prefix//'omega_xz.h5', .true., nt, ng1, st1, sz1, nh1, 'omega_xz_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(omega_xz_stat, al_z)
        call inputLine(fn_prefix//'omega_yz.h5', .true., nt, ng1, st1, sz1, nh1, 'omega_yz_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(omega_yz_stat, al_z)
        
        call inputLine(fn_prefix//'pdudx.h5', .true., nt, ng1, st1, sz1, nh1, 'pdudx_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(pdudx_stat, al_z)
        call inputLine(fn_prefix//'pdvdy.h5', .true., nt, ng1, st1, sz1, nh1, 'pdvdy_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(pdvdy_stat, al_z)
        call inputLine(fn_prefix//'pdwdz.h5', .true., nt, ng1, st1, sz1, nh1, 'pdwdz_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(pdwdz_stat, al_z)
        call inputLine(fn_prefix//'pdudz.h5', .true., nt, ng1, st1, sz1, nh1, 'pdudz_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(pdudz_stat, al_z)
        call inputLine(fn_prefix//'pdwdx.h5', .true., nt, ng1, st1, sz1, nh1, 'pdwdx_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(pdwdx_stat, al_z)
        call inputLine(fn_prefix//'up.h5', .true., nt, ng1, st1, sz1, nh1, 'up_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(up_stat, al_z)
        call inputLine(fn_prefix//'vp.h5', .true., nt, ng1, st1, sz1, nh1, 'vp_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(vp_stat, al_z)
        call inputLine(fn_prefix//'wp.h5', .true., nt, ng1, st1, sz1, nh1, 'wp_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(wp_stat, al_z)
        call inputLine(fn_prefix//'uuw.h5', .true., nt, ng1, st1, sz1, nh1, 'uuw_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(uuw_stat, al_z)
        call inputLine(fn_prefix//'vvw.h5', .true., nt, ng1, st1, sz1, nh1, 'vvw_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(vvw_stat, al_z)
        call inputLine(fn_prefix//'www.h5', .true., nt, ng1, st1, sz1, nh1, 'www_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(www_stat, al_z)
        call inputLine(fn_prefix//'uww.h5', .true., nt, ng1, st1, sz1, nh1, 'uww_stat', al_z%buffer, stat_info)
        call recoverFromAvgLine(uww_stat, al_z)
        ! call inputLine(fn_prefix//'epsilon_uu.h5', .true., nt, ng1, st1, sz1, nh1, 'epsilon_uu_stat', al_z%buffer, stat_info)
        ! call recoverFromAvgLine(epsilon_uu_stat, al_z)
        ! call inputLine(fn_prefix//'epsilon_vv.h5', .true., nt, ng1, st1, sz1, nh1, 'epsilon_vv_stat', al_z%buffer, stat_info)
        ! call recoverFromAvgLine(epsilon_vv_stat, al_z)
        ! call inputLine(fn_prefix//'epsilon_ww.h5', .true., nt, ng1, st1, sz1, nh1, 'epsilon_ww_stat', al_z%buffer, stat_info)
        ! call recoverFromAvgLine(epsilon_ww_stat, al_z)
        ! call inputLine(fn_prefix//'epsilon_uw.h5', .true., nt, ng1, st1, sz1, nh1, 'epsilon_uw_stat', al_z%buffer, stat_info)
        ! call recoverFromAvgLine(epsilon_uw_stat, al_z)
        
        ng2 = (/nx, nz/)
        st2 = (/xst(1), xst(3)/)
        sz2 = (/xsz(1), xsz(3)/)
        nh2 = (/0, 0, 0, 0/)
        fn_prefix = fn_prefix_ap_xz//trim(fn_prefix_input_stat)//'_'

        call inputPlane(fn_prefix//'Euu_kx.h5', .true., nt, ng2, st2, sz2, nh2, 'Euu_kx_stat', ap_xz%buffer, stat_info)
        call recoverFromAvgPlane(euu_kx_stat, ap_xz)
        call inputPlane(fn_prefix//'Evv_kx.h5', .true., nt, ng2, st2, sz2, nh2, 'Evv_kx_stat', ap_xz%buffer, stat_info)
        call recoverFromAvgPlane(evv_kx_stat, ap_xz)
        call inputPlane(fn_prefix//'Eww_kx.h5', .true., nt, ng2, st2, sz2, nh2, 'Eww_kx_stat', ap_xz%buffer, stat_info)
        call recoverFromAvgPlane(eww_kx_stat, ap_xz)
        call inputPlane(fn_prefix//'Euw_kx.h5', .true., nt, ng2, st2, sz2, nh2, 'Euw_kx_stat', ap_xz%buffer, stat_info)
        call recoverFromAvgPlane(euw_kx_stat, ap_xz)
        
        ng2 = (/ny, nz/)
        st2 = (/yst(2), yst(3)/)
        sz2 = (/ysz(2), ysz(3)/)
        nh2 = (/0, 0, 0, 0/)
        fn_prefix = fn_prefix_ap_yz//trim(fn_prefix_input_stat)//'_'

        call inputPlane(fn_prefix//'Euu_ky.h5', .true., nt, ng2, st2, sz2, nh2, 'Euu_ky_stat', ap_yz%buffer, stat_info)
        call recoverFromAvgPlane(euu_ky_stat, ap_yz)
        call inputPlane(fn_prefix//'Evv_ky.h5', .true., nt, ng2, st2, sz2, nh2, 'Evv_ky_stat', ap_yz%buffer, stat_info)
        call recoverFromAvgPlane(evv_ky_stat, ap_yz)
        call inputPlane(fn_prefix//'Eww_ky.h5', .true., nt, ng2, st2, sz2, nh2, 'Eww_ky_stat', ap_yz%buffer, stat_info)
        call recoverFromAvgPlane(eww_ky_stat, ap_yz)
        call inputPlane(fn_prefix//'Euw_ky.h5', .true., nt, ng2, st2, sz2, nh2, 'Euw_ky_stat', ap_yz%buffer, stat_info)
        call recoverFromAvgPlane(euw_ky_stat, ap_yz)

        if (myrank == 0) write(*,'(A)') "PowerLLEL.NOTE.inputStat: Finish reading checkpoint fields of statistics!"

        deallocate(fn_prefix)
        return
    end subroutine inputStat

    subroutine outputStat(fn_prefix_stat)
        implicit none
        character(*), intent(in) :: fn_prefix_stat
        character(len=:), allocatable :: fn_prefix
        ...
        if (myrank == 0) write(*,'(A)') "PowerLLEL.NOTE.outputStat: Writing statistics fields ..."
        ...
        sz1 = xsz(3)
        nh1 = (/0, 0/)
        fn_prefix = fn_prefix_al_z//trim(fn_prefix_stat)//string_io
        ...
        sz2 = (/xsz(1), xsz(3)/)
        nh2 = (/0, 0, 0, 0/)
        fn_prefix = fn_prefix_ap_xz//trim(fn_prefix_stat)//string_io
        ...
        sz2 = (/ysz(2), ysz(3)/)
        nh2 = (/0, 0, 0, 0/)
        fn_prefix = fn_prefix_ap_yz//trim(fn_prefix_stat)//string_io
        ...
        if (myrank == 0) write(*,'(A)') "PowerLLEL.NOTE.outputStat: Finish writing statistics fields!"
