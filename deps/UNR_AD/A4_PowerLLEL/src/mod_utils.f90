...
#ifdef _CUDA
    function Mean_d(nhalo, sz, nx_global, ny_global, dzflzi, var)
        use cudafor
        implicit none
        real(fp) :: Mean_d
        integer, dimension(6) :: nhalo
        ...
        do j = 1, sz(2)
        do i = 1, sz(1)
            Mean_d = Mean_d + var(i, j, k)*dzflzi(k)
        enddo
        enddo
        enddo
        istat = cudaDeviceSynchronize()
        call MPI_ALLREDUCE(MPI_IN_PLACE, Mean_d, 1, MPI_REAL_FP, MPI_SUM, MPI_COMM_WORLD, ierr)
        Mean_d = Mean_d/nx_global/ny_global
