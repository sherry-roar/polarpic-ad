module mod_bc_postproc
    ...
    use mod_mpi,  only: sz, halotype_vel, halotype_one, neighbor, comm_cart
    !$ use omp_lib
    ...
    private
    public :: updateBoundVel, updateBoundPre
...
contains
    ...
    subroutine updateBoundPre(nhalo, p)
        implicit none
        integer, dimension(6), intent(in) :: nhalo
        real(fp), dimension(1-nhalo(1):,1-nhalo(3):,1-nhalo(5):), intent(inout) :: p

        integer :: ibound, idir

        call updateHalo(nhalo, halotype_one, p)

        ! B.C. in x direction
        idir = 1
        if (neighbor(1) == MPI_PROC_NULL) then
            ibound = 0
            call imposePeriodicBC(nhalo, sz, ibound, idir, p)
        endif
        if (neighbor(2) == MPI_PROC_NULL) then
            ibound = 1
            call imposePeriodicBC(nhalo, sz, ibound, idir, p)
        ...
        if (neighbor(5) == MPI_PROC_NULL) then
            ibound = 0
            call imposeZeroGradBC(nhalo, sz, ibound, p)
        endif
        if (neighbor(6) == MPI_PROC_NULL) then
            ibound = 1
            call imposeZeroGradBC(nhalo, sz, ibound, p)
