...
#ifdef USE_RDMA
/* For transpose x2y/y2x */
unr_plan_h transpose_x2y_alltoall_plan, transpose_y2x_alltoall_plan;
unr_sig_h transpose_x2y_received_sig, transpose_y2x_received_sig;
...
void init_rdma_xy_alltoallv_c(const MPI_Fint decomp_2d_comm_col_f, 
    const int decomp_buf_length_f, DECOMP_2D_REAL *work1_r_f, DECOMP_2D_REAL *work2_r_f, 
    int *counts_send_x2y, int *disps_loc_send_x2y, int *disps_rmt_recv_x2y, 
    int *counts_send_y2x, int *disps_loc_send_y2x, int *disps_rmt_recv_y2x) {

    unr_init();
    unr_mem_h work1_r_h, work2_r_h;
    unr_mem_reg(work1_r_f, sizeof(DECOMP_2D_REAL) * decomp_buf_length_f, &work1_r_h);
    unr_mem_reg(work2_r_f, sizeof(DECOMP_2D_REAL) * decomp_buf_length_f, &work2_r_h);
    unr_mem_reg_sync();

    int size;
    MPI_Comm comm = MPI_Comm_f2c(decomp_2d_comm_col_f);
    MPI_Comm_size(comm, &size);
    unr_sig_create(&transpose_x2y_received_sig, size);
    unr_sig_create(&transpose_y2x_received_sig, size);

    transpose_x2y_alltoall_plan = MPI_Alltoallv_convert(
        work1_r_h, counts_send_x2y, disps_loc_send_x2y, DECOMP_2D_MPI_REAL, 
        work2_r_h, counts_send_y2x, disps_loc_send_y2x, DECOMP_2D_MPI_REAL, 
        comm, UNR_NO_SIGNAL, transpose_x2y_received_sig);
    transpose_y2x_alltoall_plan = MPI_Alltoallv_convert(
        work1_r_h, counts_send_y2x, disps_loc_send_y2x, DECOMP_2D_MPI_REAL, 
        work2_r_h, counts_send_x2y, disps_loc_send_x2y, DECOMP_2D_MPI_REAL, 
        comm, UNR_NO_SIGNAL, transpose_y2x_received_sig);
}

void rdma_x2y_alltoallv_c() {
    unr_plan_start(transpose_x2y_alltoall_plan);
}

void rdma_y2x_alltoallv_c() {
    unr_plan_start(transpose_y2x_alltoall_plan);
}

void rdma_x2y_alltoallv_wait_c() {
    unr_sig_wait(transpose_x2y_received_sig);
    unr_sig_reset(transpose_x2y_received_sig);
}

void rdma_y2x_alltoallv_wait_c() {
    unr_sig_wait(transpose_y2x_received_sig);
    unr_sig_reset(transpose_y2x_received_sig);
}
